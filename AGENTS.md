...
Keep APKs, decompiled proprietary code, phone/radio captures, identifiers and secrets in ignored local paths. Do not stage or upload `private/`, `tools/`, or historical root reports. Keep the GitHub repository private. Review the actual staged source before publication. The original Hermes index predates the cleanup fix and must not be reused.
