...
Updated September 15, 2026. Current evidence is summarized in [STATUS.md](STATUS.md). Completed hardware work does not authorize repeating it.
...
The owner plans to focus next on a mobile client and testing connection to a factory-reset shoe. Prepare the enrollment and recovery path first, then confirm readiness before the owner resets a shoe or a new key is written. No reset or hardware test is part of the current source-push task.
...
- [ ] Implement usable new-shoe setup once fresh enrollment has been validated. The current New shoes page explains this limitation.
...
The owner requested the first source push. Review actual staged contents, omit private/proprietary evidence, verify offline checks and confirm the remote result. The owner explicitly confirmed keeping the repository private for now. Public release remains a later decision. Do not describe this project as the first of its kind without evidence.
